...
async function seedDatabase() {
  ...
  if (existingActivities.length === 0) {
    ...
    await storage.createActivity({
      ...
      description: "Planificando el trabajo territorial. La marea roja está más fuerte que nunca.",
  ...
  if (existingProposals.length === 0) {
    await storage.createProposal({
      ...
      description: "Ampliación de horarios y especialistas en centros de salud municipales.",
      category: "Salud"
    });
    await storage.createProposal({
      title: "Educación Moderna",
      description: "Incorporación de tecnología y laboratorios en escuelas públicas.",
      category: "Educación"
    });
    await storage.createProposal({
      title: "Barrios Seguros",
      description: "Instalación de cámaras de seguridad y mayor patrullaje preventivo.",
      category: "Seguridad"
    });
  }
